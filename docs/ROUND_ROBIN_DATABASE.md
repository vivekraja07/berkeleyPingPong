...
```bash
# Copy the contents of sql/round_robin_schema.sql and run in Supabase SQL Editor
```
...
```bash
python scripts/import_round_robin.py --url https://berkeleytabletennis.org/results/rr_results_2025nov07
```

Import from a local HTML file:

```bash
python scripts/import_round_robin.py --file path/to/results.html
```

Dry run (parse without inserting):

```bash
python scripts/import_round_robin.py --url https://berkeleytabletennis.org/results/rr_results_2025nov07 --dry-run
```
...
```bash
python scripts/import_round_robin.py --query-player "GuangPeng Chen"
```

List all players:

```bash
python scripts/import_round_robin.py --list-players
```
